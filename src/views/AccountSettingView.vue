<template>
  <el-scrollbar height="85vh">
    <el-form :model="data" label-width="auto" label-position="top" v-if="data">
      <el-form-item label="小米账号">
        <el-input v-model="data.account" />
      </el-form-item>
      <el-form-item label="小米密码">
        <el-input v-model="data.password" type="password" />
      </el-form-item>
      <el-form-item label="关闭密码验证" label-position="left">
        <el-switch v-model="data.disable_httpauth" />
      </el-form-item>
      <template v-if="!data.disable_httpauth">
        <el-form-item label="控制台登录账户">
          <el-input v-model="data.httpauth_username" />
        </el-form-item>
        <el-form-item label="控制台登录密码">
          <el-input v-model="data.httpauth_password" />
        </el-form-item>
      </template>
      <el-button type="primary" @click="onSubmit"
        style="width: 80%;margin: 0 auto;display: flex;justify-content: center;">保存</el-button>

    </el-form>
  </el-scrollbar>
</template>
<script setup>

import { toRaw } from 'vue'
import Setting from '@/components/Setting.js';
const data = Setting.useSetting()
const emits = defineEmits(['updateSetting'])
function onSubmit() {
  Setting.saveSetting(toRaw(data.value))
  emits('updateSetting')
}
</script>